...
def opred_frak(a):  # изменение значения фракции
    global frak
    frak = a


def izm_ohi(a): # задание текста в вспомогательную строку (про ошибки и ход работы)
    global ohi
    ohi = a


def zapros_v_bazy_frakhii(a):   #   запрос инфы из базы фракции
    opred_frak(a)
    global baza_1
...
def nahalo(): # эта функция узнаёт названия баз данных
    with sql.connect('baza_Waha_1.db') as con:
        cur = con.cursor()
...
def form_spis_rosterov(): #  формирование списка ростеров
    with sql.connect('baza_Waha_1.db') as con:
        cur = con.cursor()
...
def form_spis_ynitov_v_rostere(): # формирование списка юнитов в ростере
...
def sym_prais_ros():    # подсчет итоговой цены ростера
    global itogo
    itogo = 0
    for i in ROSTER[1::]:
        itogo += i[4]
...
def slo(a):    # переключение основного слоя
...
def slo_1(a):   # переключение слоя второстепенного. нужно для 3 основного слоя
...
def formirovanie_vitrin (a, dlina):   #  формирование витрины
...
def listanie_vitrin(a):   # перелистывание витрин (нужно, если кнопки не влазят в 1 лист)
    global nomer_vitrin
    nomer_vitrin = a
    ui.setupUi(MainWindow)


def del_iz_rostera(a):   # удаление юнита из ростера
    global ROSTER
    a = a.split(' | ')
...
def ydalenie_rostera(nazv):  # удаление ростера
    with sql.connect('baza_Waha_1.db') as con:
        cur = con.cursor()
        cur.execute(f''' DELETE FROM Rostera WHERE nazv = '{nazv}' ''')


def zagryz_rostera(nazv_r):   #  загрузка ростера
    with sql.connect('baza_Waha_1.db') as con:
        cur = con.cursor()
...
def dobavl_ynita_v_roster(var, ynit):  #  добавление нового юнита в ростер
    for i in baza_1:
        if i[1] == ynit[1]:
            a = i
            break
...
def proverka_ynikal_nazv_rost(a):    #  проверка уникальности названия ростера
    with sql.connect('baza_Waha_1.db') as con:
        cur = con.cursor()
...
def vivid_bazi(a, b):   #  кнопка-селектор. обрабатывае 6 вариантов запроса с заранее не известного количества кнопок.
...
def otsev_nedopystim_povtorenii(a):     # отсев тех опций которые уже есть, и недопустимо их брать ещё.
    a_1 = []
    r = []
    for i in ROSTER[1::]:
        r.append(i[2])
...
def knopka_sohr_roster():       # сохранение ростера
    with sql.connect('baza_Waha_1.db') as con:
        cur = con.cursor()
...
def podskaz_ikanomii(a):       # вывод цены юнита. на кнопку удаления.
    a = a.split(' | ')
    with sql.connect('baza_Waha_1.db') as con:
        cur = con.cursor()
        try:
...
def nov_stroka (a):     # разбивка длинного названия на 2 строки.
    if len(a) <= 20:
        return a
